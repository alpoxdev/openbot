 ...
 * wrong about: this chart may reach an identity provider over HTTPS at hostnames,
 * and also writes the rule that says where the API may go. Those two had never
 * been compared. The server's egress named DNS, the database and the computers, so
 * on any cluster that enforces policy nobody could sign in — and the pod stayed
...
if (serverPolicy) {
  ...
  if (!/port:\s*443\b/.test(egress)) {
    problems.push(
      "The server's NetworkPolicy has no egress on 443, so the API cannot reach an identity provider. Nothing would report it: /health answers from a literal and every probe reads it.",
    );
  }
}
